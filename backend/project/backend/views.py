...
def upload_model(request):
    ...
    for file in files:
        print(os.getcwd())
        sql_path = f"{os.getcwd()}/backend/upload_model/{name}.{file.name.split('.')[1]}"
        with open(sql_path, 'wb+') as f:
            for content in file.chunks():
                # print(content)
                f.write(content)
    ...
    new_model = models.BNNmodels()
    new_model.model_name = name
    new_model.model_url = sql_path
    new_model.save()
...
def get_models(request):
    bnn_models = models.BNNmodels.objects.all()
    bnn_models = list(bnn_models)
    count = 0
    ret = []
    for bm in bnn_models:
        count += 1
        tmp = {'model_name': bm.model_name, 'model_url': bm.model_url, 'model_p': "点击此处"}
        ret.append(tmp)
    resp = {
        "code": 20000,
        "data": ret
...
def upload_data(request):
    if request.method == "POST":
        ...
        data = request.FILES.getlist("file", None)  # 接收前端传递过来的多个文件
        for d in data:
            print(os.getcwd())
            sql_path = f"{os.getcwd()}/backend/client_data/{d.name}"
            with open(sql_path, 'wb') as f:
                for content in d.chunks():
                    # print(content)
                    f.write(content)
            print(sql_path)
            sql_path = f"{os.getcwd()}/media/avatar/{d.name}"
            with open(sql_path, 'wb') as f:
                for content in d.chunks():
                    # print(content)
                    f.write(content)
